...
type Symbol struct {
	...
	MainMarginPercent     string                   `json:"mainMarginPercent"`
...
func (s *ExchangeInfoService) Do(ctx context.Context, opts ...RequestOption) (res *ExchangeInfo, err error) {
	// GET /dapi/v1/exchangeInfo | 获取交易规则和交易对
	r := &request{
...
// MaxNumberOrdersFilter define max num orders filter of symbol
type MaxNumberOrdersFilter struct {
	Limit int64 `json:"limit"`
}
...
func (s *Symbol) PercentPriceFilter() *PercentPriceFilter {
	for _, filter := range s.Filters {
		if filter["filterType"] == string(SymbolFilterTypePercentPrice) {
...
func (s *Symbol) MaxNumOrdersFilter() *MaxNumberOrdersFilter {
	for _, filter := range s.Filters {
		if filter["filterType"].(string) == string(SymbolFilterTypeMaxNumOrders) {
			f := &MaxNumberOrdersFilter{}
